...
import random
import csv
from plot import plot_varyingGamma
import os
...
if __name__ == "__main__":
    ...
    gamma_upper_lims = [4, 8, 12, 16, 20]
    
    
    for g_upper in gamma_upper_lims:
        for itr in range(max_itr):
            ...
            for ev in ev_list:
                possible_gammas = [i for i in range(g_upper-4, g_upper + 1) if i % 5 == 0] # gamma value depends on the subscription. We are taking it random
                gamma = random.choice(possible_gammas)
                ev.set_gamma(gamma)
                ev.reset_preference()
            ...
            with open(csv_file, mode='a', newline='') as file:
                ...
                if not file_exists:
                    writer.writerow(['itr', 'Method', 'gamma upper', 'Total EV', 'CP Queue', 'Total in-net CP', 'Total par-net CP', 'Total EV charged in-net', 'Total EV charged par-net','Total charge transferred in-net',
                ...
                writer.writerow([itr, 'PCG', g_upper , n_ev, q, results_PCG[0], results_PCG[1], results_PCG[2], results_PCG[3], results_PCG[4], results_PCG[5], results_PCG[6], unmatched_PCG, elapsed_time_PCG, results_PCG[7], results_PCG[8], results_PCG[9], results_PCG[10]]) 
                ...
                writer.writerow([itr, 'PCD', g_upper, n_ev, q, results_PCD[0], results_PCD[1], results_PCD[2], results_PCD[3], results_PCD[4], results_PCD[5], results_PCD[6], unmatched_PCD, elapsed_time_PCD, results_PCD[7], results_PCD[8], results_PCD[9], results_PCD[10]]) 
                ...
                writer.writerow([itr, 'PCL', g_upper, n_ev, q, results_PCL[0], results_PCL[1], results_PCL[2], results_PCL[3], results_PCL[4], results_PCL[5], results_PCL[6], unmatched_PCL, elapsed_time_PCL, results_PCL[7], results_PCL[8], results_PCL[9], results_PCL[10]]) 

    
    
    
    plot_varyingGamma(csv_file, q)
